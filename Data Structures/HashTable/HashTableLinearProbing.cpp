...
#define DEFAULT_LINEAR_CONSTANT 17
...
template <typename K, typename V>
int HashTableLinearProbing<K, V>::hashFunc(int key) {
  return abs(key) % capacity;
}

template <typename K, typename V>
int HashTableLinearProbing<K, V>::hashFunc(char key) {
  int ascii = (int)key;
  return abs(ascii) % capacity;
}

template <typename K, typename V>
int HashTableLinearProbing<K, V>::hashFunc(const char* key) {
  int keyValue = atoi(key);
  return abs(keyValue) % capacity;
}

template <typename K, typename V>
HashTableLinearProbing<K, V>::HashTableLinearProbing() {
  loadFactor = DEFAULT_LOAD_FACTOR;
  capacity = DEFAULT_CAPACITY;
  threshold = (int)capacity * loadFactor;
  usedBuckets = keysCount = 0;
  LINEAR_CONSTANT = DEFAULT_LINEAR_CONSTANT;
...
template<typename K, typename V>
int HashTableLinearProbing<K, V>::gcd(int a, int b) {
  if (a == 0)
    return b;
...
// linear probing function of type P(x) = ax + b
template<typename K, typename V>
int HashTableLinearProbing<K, V>::probeFunc(int x) {
  return LINEAR_CONSTANT * x;
}

template <typename K, typename V>
void HashTableLinearProbing<K, V>::doubleCapacity() {
  capacity = 2 * capacity;
}

template <typename K, typename V>
void HashTableLinearProbing<K, V>::adjustCapacity() {
  // P(x): probing function = ax; N = capacity
  // for P(x) to produce a full cycle MOD N
  // (a) and (N) need to be relatively prime
  // i.e. gcd(a, N) == 1
...
template <typename K, typename V>
void HashTableLinearProbing<K, V>::resizeTable() {
  // keep the old capacity value
  int oldCapacity = capacity;

  doubleCapacity(); // double the capacity
  adjustCapacity(); // adjust the linear constant and capacity value
  // reset the counts
  usedBuckets = keysCount = 0;
  threshold = (int)capacity * loadFactor;
...
template <typename K, typename V>
int HashTableLinearProbing<K, V>::size() {
  return keysCount;
}

template <typename K, typename V>
bool HashTableLinearProbing<K, V>::isEmpty() {
  return keysCount == 0;
}

template <typename K, typename V>
bool HashTableLinearProbing<K, V>::containsKey(K key) {
...
template <typename K, typename V>
V HashTableLinearProbing<K, V>::updateEntry(K key, V value) {
  int keyHash = hashFunc(key);
  int x = 1;
  int bucket = keyHash;
...
template <typename K, typename V>
V HashTableLinearProbing<K, V>::insert(K key, V value) {
  if (containsKey(key)) {
...
template <typename K, typename V>
V HashTableLinearProbing<K, V>::get(K key) {
  if(!containsKey(key)) {
...
template <typename K, typename V>
V HashTableLinearProbing<K, V>::remove(K key) {
  if(!containsKey(key)) {
    std::cerr << "\n[get()] Error! Key not found in hash table!";
    exit(0);
  }
...
template <typename K, typename V>
std::vector<K> HashTableLinearProbing<K, V>::getKeys() {
  std::vector<K> keys;
...
template <typename K, typename V>
std::vector<V> HashTableLinearProbing<K, V>::getValues() {
  std::vector<V> values;
...
template <typename K, typename V>
void HashTableLinearProbing<K, V>::printTable() {
  std::cout << "\n";
  for (int bucket = 0; bucket < capacity; bucket++) {
    if (table[bucket] != NULL && !table[bucket]->isTombstone())
